...
.lib-head{display:flex;justify-content:space-between;align-items:center;background:#fff;padding:.75rem 2rem 1rem;box-shadow:0 1px 0 #E7E5E4;flex-shrink:0;gap:1rem;flex-wrap:wrap;position:relative;z-index:1}
.lib-title{font-size:1.2rem;font-weight:700;color:#1C1917}
.lib-head img{border:none!important;outline:none;display:block}
...
// ─── LOGO ─────────────────────────────────────────────────────────────────────
// Image: 1360x880. Top half = light bg logo. Bottom half = dark bg logo.
...
// Logo content 1360x880. Top half = light bg logo. Bottom half = dark bg logo.
// Logo content (icon + wordmark, no tagline): approx x=280-990, y=90-285 in top half

function OffBookLogo({ variant="light", displayHeight=56, withTagline=false }) {
  // Crop box in image coordinates
  const cropX = 280, cropW = 710
  const cropY_top = 90
  const cropH = withTagline ? 250 : 195  // 195=wordmark only, 250=with tagline
  const darkOffset = 440  // bottom half starts here

  const scale = displayHeight / cropH
  const displayW = Math.round(cropW * scale)
  const fullImgW = Math.round(1360 * scale)
  const fullImgH = Math.round(880 * scale)
  const bgX = -Math.round(cropX * scale)
  const bgY_light = -Math.round(cropY_top * scale)
  const bgY_dark  = -Math.round((cropY_top + darkOffset) * scale)
  const bgY = variant === "dark" ? bgY_dark : bgY_light

  return (
    <div
      role="img" aria-label="OffBook"
      style={{
        width: displayW,
        height: displayHeight,
        backgroundImage: `url(${LOGO})`,
        backgroundRepeat: "no-repeat",
        backgroundSize: `${fullImgW}px ${fullImgH}px`,
      ...
      {screen === "auth" && supabase && (
        <div className="screen center-screen">
          <div className="auth-box">
            <div className="brand">
              <OffBookLogo variant="light" displayHeight={90} withTagline={true} />
            </div>
      ...
      {screen === "rehearsal" && script && (
        <div className="screen reh-screen">
          <div className="reh-bar">
            ...
            <OffBookLogo variant="dark" displayHeight={36} withTagline={false} />
